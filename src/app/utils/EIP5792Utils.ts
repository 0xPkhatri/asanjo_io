...
export function convertCapabilitiesToRecord(
  ...
  return Object.fromEntries(
    Object.entries(accountCapabilities).map(([key, value]) => [
      parseInt(key, 16),
      value,
    ])
  );
...
export function getProviderCachedCapabilities(
  ...
  const walletCapabilitiesString =
    provider.signer?.session?.sessionProperties?.["capabilities"];
...
> {
  if (provider instanceof W3mFrameProvider) {
    ...
    const mappedCapabilities = Object.entries(rawCapabilities).map(
      ([chainId]) => {
  ...
  return getFilteredCapabilitySupportedChainInfo(
    capability,
